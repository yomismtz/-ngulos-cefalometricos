...
Cada usuario crea su propio protocolo. Una instalación nueva de YomCeph **no incluye ninguna universidad, escuela, clínica, país, muestra o investigación preconfigurada**.

Cada estudio puede guardar:
...
- país e institución opcionales;
- variables configurables de procedencia, grupo o momento;
- análisis seleccionados;
- mediciones seleccionadas;
- referencia Steiner clásica, si se desea clasificación.
...
Ejemplos genéricos de variables configurables:

- `Centro = Centro A | Centro B`
- `Grupo = Caso | Control`
- `Tratamiento = Intervención | Comparador`
- `Momento = Pre | Post`

## Análisis disponibles en esta versión

- Steiner, con la opción de trabajar sin clasificación automática o con referencia clásica publicada.
...
La migración es no destructiva. Los casos, puntos, mediciones, calibraciones e historial existentes se conservan. Si una versión anterior contenía un perfil de investigación local preconfigurado, v0.12 lo trata como **Investigación importada** y elimina esa afiliación de la interfaz pública. Las instalaciones nuevas comienzan completamente neutrales.

Las nuevas investigaciones y los casos individuales se separan mediante identificadores internos sin alterar el ID visible del caso.
